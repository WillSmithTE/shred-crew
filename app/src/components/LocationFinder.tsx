        ...
        <View style={{ flex: 1, }}>
            {(() => {
                if (error) {
                    ...
                    const region = selectedPlace ? googlePlaceToRegion(selectedPlace.googlePlace, mapWidth, mapHeight) : userLocationToRegion(userLocation)
                    return <>
                        {selectedPlace && <MapView provider={Device.isDevice ? PROVIDER_GOOGLE : undefined} style={styles.map} region={region} showsUserLocation={true}>
                            {selectedPlace && <Marker coordinate={locationToLatLng(selectedPlace.googlePlace.geometry.location)} />}
                        </MapView>}
                        <KeyboardAvoidingView style={[styles.form, !selectedPlace && { marginTop: 60 }]}
                            behavior={Platform.OS === 'ios' ? 'padding' : 'height'}>
                            {initialPlace ? <>
                                <Text style={subHeader}>Are you skiing at {initialPlace.name} today?</Text>
                            ...
                            {(howAboutHere === 'no' || (yesNo1 === 'no' && places.length === 1)) && <SkiResortSelector />
                            }
                            <View style={{ flex: 1 }} />
